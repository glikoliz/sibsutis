...
{
    ui_cpu_info("Эйрих Роман Александрович ИС-141");
    return 0;
}
...
{
    int avalue=0, memvalue=0;
    int tmp=0;
    sc_accumGet(&tmp);
    sc_memoryGet(tmp, &avalue);
    sc_memoryGet(operand, &memvalue);
    sc_accumSet(avalue+memvalue);
    return 0;
}
